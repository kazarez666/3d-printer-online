...
const F={
cats:{label:'Cats',sub:'British • Siamese • Tabby • Maine Coon • Bengal',range:'$10–$50',h:1.8,b:[1.4,1.55]},
dinos:{label:'Dinosaurs',sub:'7 species • legendary Tyrannosaurus Rex',range:'$12–$80',h:2.25,b:[2.15,1.0]},
ducks:{label:'Ducks',sub:'Yellow Duck • ultra-rare Ugly Duckling',range:'$8–$250',h:1.55,b:[1.35,.95]},
...
const cats=[
{id:'british',name:'British Shorthair',rar:'Common',value:10,base:0x929ba8,kind:'british'},
{id:'siamese',name:'Siamese Cat',rar:'Common',value:10,base:0xe7dcc3,kind:'siamese'},
{id:'tabby',name:'Orange Tabby',rar:'Common',value:10,base:0xd98a43,kind:'tabby'},
{id:'maine',name:'Maine Coon',rar:'Rare',value:25,base:0x887064,kind:'maine'},
{id:'bengal',name:'Bengal Leopard Cat',rar:'Legendary',value:50,base:0xd89b42,kind:'bengal'}];
const dinos=[
...
{id:'trex',name:'Tyrannosaurus Rex',rar:'Legendary',value:80,base:0xa27149,kind:'trex'}];
...
const scene=new THREE.Scene();scene.fog=new THREE.Fog(0x08111f,10,21);
const camera=new THREE.PerspectiveCamera(34,innerWidth/innerHeight,.1,60);
const renderer=new THREE.WebGLRenderer({antialias:true,alpha:true});renderer.setPixelRatio(Math.min(devicePixelRatio,2));renderer.setSize(innerWidth,innerHeight);renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.localClippingEnabled=true;renderer.outputEncoding=THREE.sRGBEncoding;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.12;$('#scene').appendChild(renderer.domElement);
scene.add(new THREE.HemisphereLight(0xeaf1ff,0x120d0a,1.45));
const keyLight=new THREE.DirectionalLight(0xffffff,2.35);keyLight.position.set(4.8,7.5,5.2);keyLight.castShadow=true;keyLight.shadow.mapSize.set(1536,1536);keyLight.shadow.bias=-.0003;scene.add(keyLight);
const rim=new THREE.PointLight(0x6ba6ff,8,11,2);rim.position.set(-4,3.8,-3.5);scene.add(rim);
const warmLight=new THREE.PointLight(0xffbd73,3.3,8,2);warmLight.position.set(3.2,2.3,3);scene.add(warmLight);
const revealLight=new THREE.PointLight(0xffffff,0,5,2);revealLight.position.set(0,2.5,1.6);scene.add(revealLight);
const floor=new THREE.Mesh(new THREE.CircleGeometry(8,64),new THREE.MeshStandardMaterial({color:0x0e1726,roughness:.94,metalness:.03}));floor.rotation.x=-Math.PI/2;floor.position.y=-.04;floor.receiveShadow=true;scene.add(floor);
...
const bengalTex=canvasTexture((x,s)=>{x.fillStyle='#d99c45';x.fillRect(0,0,s,s);x.lineCap='round';for(let i=0;i<54;i++){const px=Math.random()*s,py=Math.random()*s,r=8+Math.random()*17,rot=Math.random()*Math.PI;x.strokeStyle='#3a2619';x.lineWidth=7;x.beginPath();x.ellipse(px,py,r*1.65,r,rot,0,Math.PI*2);x.stroke();if(i%2===0){x.fillStyle='#5a341c';x.beginPath();x.ellipse(px,py,r*.65,r*.38,rot,0,Math.PI*2);x.fill()}}});
const tabbyTex=canvasTexture((x,s)=>{x.fillStyle='#d88943';x.fillRect(0,0,s,s);x.strokeStyle='#704127';x.lineWidth=18;for(let y=-30;y<s+50;y+=55){x.beginPath();x.moveTo(-10,y);x.bezierCurveTo(s*.28,y+30,s*.72,y-20,s+10,y+8);x.stroke()}});
const rexTex=canvasTexture((x,s)=>{x.fillStyle='#9b7049';x.fillRect(0,0,s,s);for(let i=0;i<700;i++){const a=.04+Math.random()*.14;x.fillStyle=`rgba(40,33,23,${a})`;const r=1+Math.random()*4;x.beginPath();x.arc(Math.random()*s,Math.random()*s,r,0,Math.PI*2);x.fill()}});
const mkMat=(color,opts={})=>new THREE.MeshStandardMaterial({color,map:opts.map||null,roughness:opts.roughness??.38,metalness:opts.metalness??.015,clippingPlanes:[clip],emissive:opts.emissive||0x000000,emissiveIntensity:opts.emissiveIntensity||0});
...
const organic=(g,m,pos,scale,seg=36)=>part(g,new THREE.SphereGeometry(1,seg,Math.max(20,seg*.66|0)),m,pos,scale);
const limb=(g,m,a,b,r=.08)=>{const A=new THREE.Vector3(...a),B=new THREE.Vector3(...b),mid=A.clone().add(B).multiplyScalar(.5),len=A.distanceTo(B),q=new THREE.Mesh(new THREE.CylinderGeometry(r*.78,r,len,18),m);q.position.copy(mid);q.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),B.clone().sub(A).normalize());q.castShadow=q.receiveShadow=true;g.add(q);return q};
const tube=(g,m,pts,r=.06)=>{const q=new THREE.Mesh(new THREE.TubeGeometry(new THREE.CatmullRomCurve3(pts.map(p=>new THREE.Vector3(...p))),42,r,12,false),m);q.castShadow=q.receiveShadow=true;g.add(q);return q};
const eye=(g,pos,color=0x111318,s=.04)=>organic(g,mkMat(color,{roughness:.12}),pos,[s,s*.78,s*.55],20);
function loft(g,m,rings,axis='z',segments=28){const pos=[],uv=[],idx=[];const n=rings.length;for(let i=0;i<n;i++){const r=rings[i];for(let j=0;j<segments;j++){const a=j/segments*Math.PI*2,c=Math.cos(a),sn=Math.sin(a);let x,y,z;if(axis==='z'){x=(r.cx||0)+c*r.rx;y=(r.cy||0)+sn*r.ry;z=r.p}else{x=r.p;y=(r.cy||0)+sn*r.ry;z=(r.cz||0)+c*r.rz}pos.push(x,y,z);uv.push(j/segments,i/(n-1))}}for(let i=0;i<n-1;i++)for(let j=0;j<segments;j++){const a=i*segments+j,b=i*segments+(j+1)%segments,c=(i+1)*segments+j,d=(i+1)*segments+(j+1)%segments;idx.push(a,c,b,b,c,d)}const geo=new THREE.BufferGeometry();geo.setAttribute('position',new THREE.Float32BufferAttribute(pos,3));geo.setAttribute('uv',new THREE.Float32BufferAttribute(uv,2));geo.setIndex(idx);geo.computeVertexNormals();return part(g,geo,m,[0,0,0])}
